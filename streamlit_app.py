"""
earth_science_app.py — 地球科學測驗網頁（Streamlit Cloud + Firebase）
難度：國中、高中
每題限時：30 秒
"""
...
def track_visitor(site_id: str) -> int:
    """
    負責「計數」與「回傳目前訪客數」兩件事：
    - 計數：用 Firebase 的 .transaction() 做原子遞增，確保多人同時
      觸發時不會互相覆蓋、漏算。
...
SITE_ID = "site_earth_science_examine"
...
def save_record(name: str, score: int, difficulty: str) -> str:
    ...
    firebase_db.reference("records_earth_science").push({
        "id":         record_id,
        "name":       name,
        "score":      score,
...
def load_records_cached(difficulty: str) -> list:
    ...
    先行過濾，避免每次都把整個 records_earth_science 全部下載回來。
    需搭配 Firebase Rules 的 ".indexOn": ["difficulty"] 才有索引效能。
    """
    init_firebase()
    ref  = firebase_db.reference("records_earth_science")
...
st.set_page_config(page_title="地球科學測驗挑戰網", page_icon="🌍", layout="centered")
...
.score-box   { font-size:3rem; font-weight:900; text-align:center; color:#1565c0; margin:1rem 0; }
.champ-name  { font-size:1rem; font-weight:700; color:#1565c0; }
...
if st.session_state.step == "login":
    st.markdown('<div class="big-title">🌍 地球科學測驗挑戰網</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">測試你的地球科學實力，挑戰榮譽榜！</div>', unsafe_allow_html=True)
        ...
        if not records:
            ...
            if not top10:
                ...
                for i, r in enumerate(top10):
                    ...
                    st.markdown(
                        f"{medal} &nbsp; **{r['name']}** &nbsp; "
                        f"<span style='color:#1565c0;font-weight:700'>{r['score']} 分</span>"
